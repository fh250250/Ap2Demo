...
public class AirPlayBonjour {
    ...
    private static final String FAKER_MAC = "01:23:45:67:89:ab";
    ...
    public void start(int airTunesPort) throws Exception {
        Enumeration<NetworkInterface> ifas = NetworkInterface.getNetworkInterfaces();

        while (ifas.hasMoreElements()) {
            NetworkInterface ifa = ifas.nextElement();

            if (!ifa.isLoopback() &&
                    !ifa.isPointToPoint() &&
                    !ifa.isVirtual() &&
                    ifa.isUp() &&
                    ifa.supportsMulticast()) {
                Enumeration<InetAddress> inetAddresses = ifa.getInetAddresses();
